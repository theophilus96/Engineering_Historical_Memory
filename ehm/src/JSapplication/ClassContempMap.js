...
export class ContempMap {
    constructor(container, mapType, totalResults = [], clickAction, searchAction, maxClusterLayer, initialLayer, initialCenter, currentVis) {
        ...
        this.mapContainer = document.getElementById(this.container);
        this.globeContainer = document.getElementById('globe_container');
        this.visContainer = document.getElementById('visualization_container');
    ...
    initMap() {
        this.contMap = L.map(this.container, {preferCanvas: true}).setView(this.initialCenter, this.initialLayer);
        this.changeToVisual('sat');
        this.createSearchBar();
        this.createVisualizationButton(this.container, this.mapType);
        this.createNotAvail();
    ...
    createVisualizationButton(contmap_container, map_type) {
        ...
        if (MAP_PAGES.includes(map_type)) {
            let option1 = document.createElement('option');
            option1.value = 'rad';
        ...
        selectDom.addEventListener('change',  ()=> {
            ...
            if(this.changeToVisual) this.changeToVisual(vis);
            // setParams({vis: vis, cid: '', hid: '', coun_id: ''});
            setParams({vis: vis});
        });
        document.getElementById(contmap_container).parentElement.appendChild(container);
    }
    ...
    changeMapToGlobe(vis) {
        ...
        this.globeMap = new GlobeMap('globe_container', this.mapType, this.totalResults, this.clickAction, null, null, null, null, this.initialLayer, this.initialCenter, vis);
    }
    ...
    initialLoad(results, mode = 1, clicked) {
