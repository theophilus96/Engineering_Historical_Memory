...
import {loadContempInfo, loadHistInfo, loadAllInfo} from "./loadResources1.js";
import {Draggable} from "./ClassDraggable.js";
import {FraMauroMap} from "./ClassFraMauroMap1.js";

function histMapClickAction(result, clickPopup) {
    ...
    clickPopup.setContTitle(result[cont_place_name_field]);


    var identified = contempMapPopup(result, contempMap, suggestModal, hist_id_field, cont_id_field);
    // var identified = true;
    if (contempMap && contempMap.visualization) contempMap.visualization.popup(result);
    loadAllInfo(MAP_TYPE, result, identified, clickPopup.credit);
    // loadHistInfo(MAP_TYPE, result, creditCont);
}

function contempMapClickAction(results) {
    setParams({hid: '', cid: results[0][cont_id_field]});
    if (multiSpectralCont) multiSpectralCont.style.display = 'none';
    multiSpectralStatus = 0;
    histMapPopup(histMap, results);
    loadAllInfo(MAP_TYPE, results[0], true, histMap.clickPopups[0].credit);
}
...
function infographAction(totalResults) {
    infographDraggable.show();
    if (contempMap) return;
    contempMap = new ContempMap(contMapCont, MAP_TYPE, totalResults, contempMapClickAction, contempMapSearchAction,
        null, null, null, vis,
        globeMapCont, visCont, true, visClickAction, visResetAction);
    contempMap.reload(totalResults);
    // contempMap.centerMapOnResults(this.results, false);
}

function ehmSearchAction() {
    ehmSearchDraggable.show();
}
...
function visClickAction(data, fullNodes = null) {
    ...
    if (data.hasOwnProperty('hist_id')) {
        ...
        if (fullNodes) results = fullNodes.filter(node => node.result && node.result[cont_country_field] === data.country_id).map(node => node.result);
            ...
            results = contempMap.visualization.totalResults.filter(r => cont_ids.includes(r[cont_id_field]));
        }
    ...
    histMapPopup(histMap, results);
    loadHistInfo(MAP_TYPE, results[0], histMap.clickPopups[0].credit);
    if (multiSpectralCont) multiSpectralCont.style.display = 'none';
    multiSpectralStatus = 0;
}
...
function initialLoad(results) {
    ...
    // contempMap.initialLoad([], mode, cont_clicked);
    if (cont_clicked && contempMap && contempMap.visualization) contempMap.visualization.popup(results[0]);
    if (coun_id) histMapPopup(histMap, results);
    loadAllInfo(MAP_TYPE, results[0]);
}

function pageHandle(totalResults) {
    histMap = new FraMauroMap('hist_map_container', `${DOMAIN}/Zoomify/Fra_Mauro_Map_Zoomify/`, totalResults, histMapClickAction,
        histMapSearchAction, histMapFilterAction, changeDisplayAction, infographAction, ehmSearchAction, introAction,
        [22110, 22110], MAP_TYPE, dis);
    ...
    // visualization = new Visualization(visCont.id, MAP_TYPE, totalResults, visClickAction, visResetAction);
...
function multiSpectralChooseMapAction(zoomifyPath) {
    ...
    var histMap = new ol.Map({
        layers: [layer],
        target: multiSpectralDraggable.id,
        view: new ol.View({
...
function multiSpectralAction() {
    if (multiSpectralStatus) {
        ...
        if (multiSpectralCont) multiSpectralCont.style.display = 'block';
        loadAllInfo('fm_spec');
    }
    ...
    if (multiSpectralCont) multiSpectralCont.appendChild(createMultiSpectralBtn('Back to map explorer', 'return.png'));
}
...
var multiSpectral, multiSpectralStatus = 0, multiSpectralDraggable = document.createElement('div');
var histMap, contempMap;
const multiSpectralCont = document.getElementById("multiSpectralCont");

const contMapCont = document.createElement("div");
contMapCont.id = "contMapCont";
const globeMapCont = document.createElement("div");
globeMapCont.id = "globeMapCont";
const visCont = document.createElement("div");
visCont.id = "visCont";

const infographDraggable = new Draggable(document.getElementById("infographDraggable"), false, 2 / 3);
infographDraggable.body.append(contMapCont, globeMapCont, visCont);

const ehmSearchCont = document.createElement("div");
ehmSearchCont.id = "ehm_info";
const ehmSearchDraggable = new Draggable(document.getElementById("ehmSearchDraggable"), false, 2 / 3);
ehmSearchDraggable.body.appendChild(ehmSearchCont);

const infoCont = document.createElement("div");
infoCont.id = "cartouche_info";
const introDraggable = new Draggable(document.getElementById("introDraggable"), false);
introDraggable.body.appendChild(infoCont);

// introDraggable.body.appendChild(contMapCont);
// var contempMap = new ContempMap('cont_map_container1', MAP_TYPE, [], contempMapClickAction,
...
var draggable = new Draggable(document.getElementById('shipDraggable'));
