...
import {FullPopup} from "./ClassPopup1.js"
import {createBtnWithTooltip} from "./createBtn.js";
import {Widget} from "./ClassWidget.js";

const TWO_DISPLAY_MAPS = ['fgm', 'imola', 'cadastral', 'sinai'];
const FILTER_BY_NAME_MAPS = ['imola', 'cadastral', 'sinai'];
...
export class HistMap {
    constructor(container, zoomifyUrl, totalResults, clickAction, searchAction, filterAction, changeDisplayAction,
                infographAction, ehmSearchAction, introAction,
                imgDim, fieldType, currentDisplay) {
        const map_fields = FIELDS[fieldType];
        ...
        this.hist_cartouche_eng_field = map_fields.hist_cartouche_eng;
        ...
        this.infographAction = infographAction;
        // this.ehmSearchAction = ehmSearchAction;
        ...
        this.maxLayer = fieldType === 'gm' ? 4 : fieldType === 'sinai' ? 3 : 4;
        this.minLayer = 1;
        [this.imgWidth, this.imgHeight] = imgDim;
        this.fieldType = fieldType;
        this.zoomifyUrl = zoomifyUrl;
        this.distance = 100;
        this.initialCenter = fieldType === 'sinai' ? [this.imgWidth / 2, 0] : [this.imgWidth / 2, -this.imgHeight / 2];
        this.noDisplayModes = 3;

        this.histMap = this.initMap();
        this.createChangeDisplayButton();
        this.createInfoGraphicBtn();
        // this.createEHMSearchBtn();
        this.createIntroBtn();
    ...
    createInfoGraphicBtn() {
        var [btn, tooltip, container] = this.createBtn('Infographics', '', 'topright');
        container.className = 'infographicsBtn ol-unselectable ol-control';

        btn.onclick = () => {
            // this.infographicWidget.setPosition([20,20])
            this.infographAction(this.totalResults);
        }

        this.histMap.addControl(new ol.control.Control({element: container}));
    }

    createEHMSearchBtn() {
        var [btn, tooltip, container] = this.createBtn('EHM Search', '', 'topright');
        container.className = 'ehmSearchBtn ol-unselectable ol-control';

        btn.onclick = () => {
            this.ehmSearchAction();
        }
    ...
    createIntroBtn() {
        var [btn, tooltip, container] = this.createBtn('Intro', '', 'topright');
        container.className = 'introBtn ol-unselectable ol-control';
    ...
    createCategoryFilter() {
        ...
        filterSelect.onchange = function () {
            ...
            if (selectedCategory === 'all') var filterResults = _thisRef.totalResults;
            else filterResults = _thisRef.totalResults.filter(result => result[_thisRef.hist_categories_field].toLowerCase().split("|").includes(selectedCategory));
            let selectedCategoryEncoded = encodeURI(selectedCategory.toLowerCase());
            _thisRef.setSearchBox();
            _thisRef.searchResults = filterResults;
            _thisRef.currentResults = [];
            _thisRef.filterAction(filterResults, selectedCategoryEncoded);
        };
    ...
    createClickPopups() {
        var clickPopups = [];
        for (let i = 0; i < this.clickPopupNumber; i++) {
            let clickPopup = new ClickPopup(this.fieldType, this.histMap, this.totalResults);
            clickPopups.push(clickPopup);
            // clickPopup.title.onclick = () => {
            //     clickPopup.showCredit();
            // }
            // clickPopup.trove.onclick = ()=>{
            // }
        }
        return clickPopups;
    }
    ...
    popupMarkerOnClick(marker) {
        ...
        var clickPopup = this.popupMarker(result, 0, yzoom / this.imgHeight < 0.1);
        this.clusters.setStyle(source => this.getStyle(source.get('features'), null, this.chosenIds));
        // clickPopup.credit.style.display = 'none';
        // clickPopup.title.style.display = clickPopup.categories.style.display = 'block';
        clickPopup.showTitle();
        this.clickAction(result, clickPopup);
    }
    ...
    popupMarker(result, clickPopupIndex = 0, bottom = false) {
        ...
        var credit = result[this.hist_cartouche_eng_field];
        ...
        clickpopup.setCredit(credit);
        clickpopup.setResults([result]);
    ...
    moveHandle(evt) {
        ...
        if (feature) {
            ...
            if(this.movePopup) this.movePopup.hide();
    ...
    async markerClick(hid, clicked) {
        ...
        var clickpopup = this.popupMarker(chosenResult)
        this.centerMapOnResults([chosenResult]);
        this.clickAction(chosenResult, clickpopup);
    }
...
class ClickPopup extends FullPopup {
    constructor(map_type, histMap, totalResults) {
        super();
...
class MovePopup extends Popup {
    ...
    createContent() {
        ...
        var trove = this.trove = document.createElement('div');
...
class HistWidget extends Widget {
    widget_init(histMap) {
