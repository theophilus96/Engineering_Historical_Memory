...
import {Citation} from "./ClassCitation1.js";
...
export function loadAllInfo(map_type, result, identified = true, creditCont) {
...
export function loadCredit(map_type, result, cont) {
    ...
    if (!credit) {
        ...
        getRequest(`controller/getCredit.php`, {name: map_type}, res => {
            res = JSON.parse(res);
            if (cont) cont.innerHTML = `<p style='text-align: center;font-size: large;font-weight: bold'>${res.title}</p>${res.content}`;
...
export function loadContempInfo(map_type, result = null, identified = true) {
    if (map_type === 'collab') {
        ...
        place_name_split = result ? (result[cont_place_name_split_field] ? result[cont_place_name_split_field] : result[post_title_field]) : null;
    }
    var wiki_url = result ? (result[hist_wiki_url_field] ? result[hist_wiki_url_field] : result[cont_wiki_url_field]) : null;
    // var resources = result ? result[cont_resources_field] : null;
...
function loadFMCartouche(map_type, result) {
    ...
    if (hist_place_name) html += `<p class="popup-title"><b>${hist_place_name}</b></p>`;
    if (categories) html += `<p class="popup-cat">${categories.split("|").map(c => c.charAt(0).toUpperCase() + c.slice(1)).join(", ")}</p>`;
...
function loadWikipediaInfo(map_type, wiki_url = null, place_name_encoded = null, identified = true) {
    if (wiki_url) wiki_url = wiki_url.replace('.wikipedia', '.m.wikipedia');
    else {
    ...
    const wikiCont = document.getElementById('popup_wikipedia');
    if (wikiCont) wikiCont.innerHTML = `<iframe src='${wiki_url}'></iframe>`;
}
...
function loadResourcesMashup(map_type, place_name_split) {
    if (!mashup) mashup = new ResourcesMashup('popup-mashup', map_type, place_name_split);
