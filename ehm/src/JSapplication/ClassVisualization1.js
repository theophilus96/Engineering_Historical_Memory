...
export class Visualization {
    constructor(container, mapType, totalResults, clickAction, resetAction, boschKeywords, visSelect) {
        ...
        this.chart.on("click", (node) => {
            if (this.mode == 'rad') {
                if (this.mapType === 'sinai') {
                    ...
                    if (this.clickAction) this.clickAction(node.data.hist_ids);
                    ...
                    if (this.clickAction) this.clickAction(keyword_item);
                    ...
                    if (this.clickAction) this.clickAction(country_item);
                } else if (node.data.hasOwnProperty('cont_id')) {
                    let cont_item = this.addRemoveHistNodes(this.data, node.data.cont_id);
                    if (this.clickAction) this.clickAction(cont_item);
                } else if (node.data.hasOwnProperty('hist_id') && this.clickAction) this.clickAction(node.data);
                this.showGraph(this.data);
                this.closeNotAvail();
            } else if (this.mode == 'tmp') {
                if (this.visSelect.value != 'tmp') return;
                if (this.clickAction) this.clickAction(node.data);
                ...
                if (this.clickAction) this.clickAction(node.data, this.fullNodes);
            }
    ...
    load() {
        this.mode = this.visSelect.value;
    ...
    createNotAvail() {
        ...
        this.container.appendChild(this.notAvailBtn);
    }
